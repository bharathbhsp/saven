...
function userCategoriesGroupId(userId) {
  return USER_CATEGORIES_PREFIX + userId;
}

async function ensureUserDefaults(userId) {
  const gid = userCategoriesGroupId(userId);
  const r = await doc.query({
    TableName: TABLES.categories,
    KeyConditionExpression: "groupId = :gid",
...
// --- Per-user categories (groupId = USER#userId) ---

async function listForUser(params, body, userId) {
  await ensureUserDefaults(userId);
  const gid = userCategoriesGroupId(userId);
  const items = await queryAll(TABLES.categories, "groupId = :gid", { ":gid": gid });
  const categories = items.filter((c) => !c.archived);
  return json(200, { categories });
}

async function createForUser(params, body, userId) {
  const name = body && body.name;
  if (!name || typeof name !== "string" || !name.trim()) return badRequest("name is required");
  const categoryId = uuid();
  const ts = now();
...
async function getForUser(params, body, userId) {
  const { categoryId } = params;
  const gid = userCategoriesGroupId(userId);
  const r = await doc.get({
    TableName: TABLES.categories,
    Key: { groupId: gid, categoryId },
  }).promise();
  if (!r.Item) return notFound("Category not found");
  return json(200, { category: r.Item });
}

async function updateForUser(params, body, userId) {
  const { categoryId } = params;
  const gid = userCategoriesGroupId(userId);
  const r = await doc.get({
    TableName: TABLES.categories,
...
async function archiveForUser(params, body, userId) {
  return updateForUser({ ...params }, { ...body, archived: true }, userId);
}
